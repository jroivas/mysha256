...
void Hash::round(std::unique_ptr<sha::Message::Chunk> &chunk)
{
    storedDigest = "";
    uint32_t r[8];
    memcpy(r, hash, 8 * 4);
...
void Hash::round(std::vector<std::unique_ptr<sha::Message::Chunk>> &chunks)
{
...
{
    reset();
    std::vector<std::unique_ptr<sha::Message::Chunk>> chunks = sha::Message::Chunk::create(str);
    round(chunks);
}
